...
describe("AnthropicProvider", () => {
	...
	describe("transformRequest", () => {
		it("translates basic user message to Anthropic content array format", () => {
			...
			const result = provider.transformRequest("claude-3-5-sonnet-20241022", messages, { modify_params: true });
	...
	describe("DIFF-ANTH-02: _mapReasoningEffort 兜底 (对齐 PY transformation.py:1436-1448)", () => {
		it("max effort 兜底返回 HIGH budget_tokens (非 Opus 4.6 时)", () => {
			const result = (
				provider as unknown as {
					_mapReasoningEffort: (e: string | undefined | null, m: string) => unknown;
				}
			)._mapReasoningEffort("max", "claude-sonnet-4-5");
			expect(result).toEqual({ type: "enabled", budget_tokens: 4096 });
		});

		it("max effort 对 Opus 4.6 返回 adaptive", () => {
			const result = (
				provider as unknown as {
					_mapReasoningEffort: (e: string | undefined | null, m: string) => unknown;
				}
			)._mapReasoningEffort("max", "claude-opus-4-6");

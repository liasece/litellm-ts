 ...
 * GLM 提供商（智谱 BigModel）
 *
 * 使用 Anthropic 兼容协议处理 GLM 系列模型的请求/响应转换。
 */
export class GLMProvider implements ProviderConfig {
	private _apiBase: string;

	constructor(apiBase = "https://open.bigmodel.cn/api/paas/v4") {
	...
	transformRequest(model: string, messages: Message[], optionalParams: Record<string, unknown>): ProviderRequest {
		const cleanModel = this.stripModelPrefix(model);
		...
		if (optionalParams.tools) {
			body.tools = this.mapTools(optionalParams.tools as unknown[]);
		}
	...
	transformResponse(
		...
		const id = (raw.id as string) ?? `glm-${Date.now()}`;
	...
	/**
	 * 解析 SSE 流响应（Anthropic SSE 格式），生成 ModelResponseStream 块
	 * @param response
	 ...
	 * @param model
	 */
	stripModelPrefix(model: string): string {
		return model.replace(/^(glm-|chatglm-)/i, "");
	}
	...
	/**
	 * 映射工具列表，特殊处理 web_search
	 * @param tools
	 */
	mapTools(tools: unknown[]): unknown[] {
		return tools.map((t) => {
			const tool = t as Record<string, unknown>;
			if (tool.type === "web_search") {
				return {
					type: "web_search",
					web_search: (tool.web_search as Record<string, unknown>) ?? {},
				};
			}
			return tool;
		});

...
export const liteLLM_DailyTeamSpend = pgTable(
	...
	(table) => ({
		unq: uniqueIndex("daily_team_spend_unique").on(
			table.team_id,
			table.date,
			table.api_key,
			table.model,
		...
		dateIdx: index("daily_team_spend_date").on(table.date),
		teamDateIdx: index("daily_team_spend_team_date").on(table.team_id, table.date),
		apiKeyIdx: index("daily_team_spend_api_key").on(table.api_key),
		modelIdx: index("daily_team_spend_model").on(table.model),
		mcpToolIdx: index("daily_team_spend_mcp_namespaced_tool_name").on(table.mcp_namespaced_tool_name),
		endpointIdx: index("daily_team_spend_endpoint").on(table.endpoint),
	}),
);

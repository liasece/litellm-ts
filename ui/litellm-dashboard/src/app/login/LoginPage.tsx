	...
	useEffect(() => {
		...
		void (async () => {
			...
			if (ssoCode) {
				...
				await exchangeLoginCode(ssoCode, workerUrl);
				params.delete("code");
			...
			if (params.has("worker") && uiConfig?.is_control_plane) {
			...
			try {
				await getWebUiSession();
				router.replace(consumeReturnUrl() || "/ui");
				return;
			} catch {
				// 无有效 HttpOnly session，继续展示登录或跳转 SSO。
			}

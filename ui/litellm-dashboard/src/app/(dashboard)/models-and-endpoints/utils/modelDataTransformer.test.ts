...
describe("transformModelData", () => {
	...
	it("should transform cache read token cost per 1M without mutating source data", () => {
		const rawData = {
			data: [
				{
					model_name: "cached-model",
					litellm_params: { model: "openai/cached-model" },
					model_info: { cache_read_input_token_cost: 0.00000025 },
				},
			],
		};
	...
	it("should retain zero cache read cost as 0.00 and missing cost as null", () => {
		const result = transformModelData(
			{
				data: [
					{ model_name: "zero", litellm_params: { model: "zero" }, model_info: { cache_read_input_token_cost: 0 } },
					{ model_name: "missing", litellm_params: { model: "missing" }, model_info: {} },
				],
			},

...
export const transformModelData = (rawModelData: any, getProviderFromModel: (model: string) => string) => {
	...
	for (let i = 0; i < transformedData.length; i++) {
		...
		let provider = "";
		let input_cost: number | null = null;
		let output_cost: number | null = null;
		let cache_read_input_cost: number | null = null;
		...
		if (model_info) {
			input_cost = model_info?.input_cost_per_token ?? null;
			output_cost = model_info?.output_cost_per_token ?? null;
			cache_read_input_cost = model_info?.cache_read_input_token_cost ?? null;
		...
		transformedData[i].cache_read_input_cost = cache_read_input_cost;

...
const mockAllDeploymentHealthCheckCall = vi.fn().mockResolvedValue({
	healthy_count: 1,
	unhealthy_count: 0,
	healthy_endpoints: [{ model_id: "model-health-1", status: "healthy" }],
	unhealthy_endpoints: [],
});
		...
		const modelData = createPaginatedModelData([
			{
				...
				litellm_params: { model: "health-model" },
				model_info: { id: "model-health-1", db_model: true, direct_access: true, access_groups: [] },
			},
		]);
		mockUseModelsInfo.mockReturnValue({ data: modelData, isLoading: false, error: null, refetch: vi.fn() });
		renderWithProviders(<AllModelsTab {...defaultProps} />);

		await waitFor(() => expect(screen.getByText("Health Status")).toBeInTheDocument());
		fireEvent.click(screen.getByRole("button", { name: "Run All Checks" }));

		await waitFor(() => expect(mockAllDeploymentHealthCheckCall).toHaveBeenCalledWith("mock-access-token"));

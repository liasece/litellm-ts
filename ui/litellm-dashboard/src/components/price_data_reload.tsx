...
const PriceDataReload: React.FC<PriceDataReloadProps> = ({
	...
	const fetchReloadStatus = React.useCallback(async () => {
	...
	useEffect(() => {
		void Promise.resolve().then(fetchReloadStatus);
		void Promise.resolve().then(fetchSourceInfo);

		const interval = setInterval(() => {
			void fetchReloadStatus();
			void fetchSourceInfo();
		}, 30000);

		return () => clearInterval(interval);
	}, [fetchReloadStatus, fetchSourceInfo]);

...
const MCPServerEdit: React.FC<MCPServerEditProps> = ({
	...
	const fetchTools = React.useCallback(async () => {
		...
		mcpServer.args,
		mcpServer.auth_type,
		mcpServer.authorization_url,
		mcpServer.command,
		mcpServer.env,
		mcpServer.mcp_info,
		mcpServer.registration_url,
		mcpServer.server_id,
	...
	// but only after the server has a permanent ID.
	useEffect(() => {
		if (!mcpServer.server_id || mcpServer.server_id.trim() === "") {
			return;
		}
		void Promise.resolve().then(fetchTools);
	}, [fetchTools, mcpServer.server_id]);

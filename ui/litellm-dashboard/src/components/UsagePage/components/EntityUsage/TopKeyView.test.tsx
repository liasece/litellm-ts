...
vi.mock("../TopRankingBarChart", () => ({
	default: ({ data }: { data: Array<{ key_alias: string }> }) => (
		<div>{data.map((item) => item.key_alias).join(",")}</div>
	),
}));
...
describe("TopKeyView", () => {
	...
	it("shows all keys beyond the former limit in table and chart views", async () => {
		const user = userEvent.setup();
		const topKeys = Array.from({ length: 12 }, (_, index) => ({
			api_key: `key-${index + 1}`,
			key_alias: `Key ${index + 1}`,
			spend: 12 - index,
		}));
		render(<TopKeyView {...baseProps} topKeys={topKeys} />);
		expect(screen.getByText("Key 12")).toBeInTheDocument();

		await user.click(screen.getByRole("button", { name: "Chart View" }));
		expect(
			screen.getAllByText((_, element) => element?.textContent?.includes("Key 12") ?? false).length,
		).toBeGreaterThan(0);
	});

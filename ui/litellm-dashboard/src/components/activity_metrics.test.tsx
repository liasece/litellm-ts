...
vi.mock("@tremor/react", () => ({
	...
	BarChart: ({
		...
		return (
			<div data-testid="bar-chart" data-category={category} data-layout={layout} style={style}>
				BarChart
				{data?.map((item) => {
					const value = category ? item[category] : undefined;
					return (
						<span key={String(item[index || ""])}>
							<span>{String(item[index || ""])}</span>
							<span>{typeof value === "number" && valueFormatter ? valueFormatter(value) : ""}</span>
						</span>
					);
				})}
			</div>
...
vi.mock("./UsagePage/utils/value_formatters", () => ({
	...
	valueFormatterTokens: (value: number) => value.toLocaleString("en-US", { maximumFractionDigits: 0 }),
}));
...
vi.mock("@/utils/dataUtils", () => ({
	formatNumberWithCommas: (value: number, decimals: number = 0) =>
...
const createMockKeyMetricWithMetadata = (
	metadata: { key_alias: string | null; key_name?: string | null; team_id: string | null },
...
describe("ActivityMetrics", () => {
	...
	it("should display top API keys section when present", () => {
		...
		expect(screen.getAllByText("Test Key (team_id: team1)")).toHaveLength(2);
		expect(screen.getAllByTestId("bar-chart").some((chart) => chart.dataset.layout === "vertical")).toBe(true);
	});

	it("renders full spend and token virtual-key charts with independent sorting and responsive sizing", () => {
		const topApiKeys = Array.from({ length: 6 }, (_, index) => ({
			api_key: `key-${index + 1}`,
			key_alias: `Key ${index + 1}`,
			team_id: null,
			spend: index + 1,
			requests: index + 1,
		...
		const { container } = render(
			<ActivityMetrics
				modelMetrics={{
					"gpt-4": createMockModelActivityData("GPT-4", { top_api_keys: topApiKeys }),
				}}
			/>,
		);
		...
		const spendChart = screen
			...
			.find((chart) => chart.dataset.category === "spend" && chart.textContent?.includes("Key 6"));
		const tokenChart = screen
			.getAllByTestId("bar-chart")
			.find((chart) => chart.dataset.category === "tokens" && chart.textContent?.includes("Key 1"));
		const spendText = spendChart?.textContent || "";
		const tokenText = tokenChart?.textContent || "";

		expect(screen.getByText("Top Virtual Keys by Tokens")).toBeInTheDocument();
		expect(spendText.indexOf("Key 6")).toBeLessThan(spendText.indexOf("Key 1"));
		expect(tokenText.indexOf("Key 1")).toBeLessThan(tokenText.indexOf("Key 6"));
		expect(topApiKeys.every((key) => spendText.includes(key.key_alias))).toBe(true);
		expect(topApiKeys.every((key) => tokenText.includes(key.key_alias))).toBe(true);
		expect(tokenText).toContain("74,070");
		expect(topApiKeys.map((key) => key.key_alias)).toEqual(["Key 1", "Key 2", "Key 3", "Key 4", "Key 5", "Key 6"]);
	...
	it("gives a single virtual key enough chart height for axes and its bar", () => {
		const modelWithOneKey: Record<string, ModelActivityData> = {
			"gpt-4": {
				...mockModelMetrics["gpt-4"],
				top_api_keys: [
					{
		...
		render(<ActivityMetrics modelMetrics={modelWithOneKey} />);

		const keyChart = screen
			.getAllByTestId("bar-chart")
			.find((chart) => chart.dataset.layout === "vertical" && chart.textContent?.includes("Only Key"));
		expect(keyChart).toHaveStyle({ height: "74px" });
	});
	...
	it("should display API key hash when alias is missing", () => {
		...
		render(<ActivityMetrics modelMetrics={modelWithTopKeys} />);
		expect(screen.getAllByText(/key-123456/)).toHaveLength(2);
	});
	...
	it("should display team information for top API keys", () => {
		...
		render(<ActivityMetrics modelMetrics={modelWithTopKeys} />);
		expect(screen.getAllByText(/\(team_id: team1\)/)).toHaveLength(2);
	});

	it("uses alias then name then truncated hash and disambiguates duplicate labels", () => {
		const modelWithTopKeys: Record<string, ModelActivityData> = {
			"gpt-4": {
				...mockModelMetrics["gpt-4"],
				top_api_keys: [
					{
						api_key: "first-hash-one",
						key_alias: "Shared",
						key_name: "Ignored",
						team_id: null,
						spend: 4,
					...
					{
						api_key: "hash-only-secret-value",
						key_alias: null,
						key_name: null,
						team_id: null,
						spend: 1,
		...
		expect(screen.getAllByText("Shared (first-ha)")).toHaveLength(2);
		expect(screen.getAllByText("Shared (second-h)")).toHaveLength(2);
		expect(screen.getAllByText("Named Key")).toHaveLength(2);
		expect(screen.getAllByText("hash-only-...")).toHaveLength(2);
		expect(screen.queryByText("Ignored")).not.toBeInTheDocument();
		expect(screen.queryByText("hash-only-secret-value")).not.toBeInTheDocument();
	});
...
describe("processActivityData", () => {
	...
	it("should retain all API keys sorted by spend", () => {
		...
		expect(result["gpt-4"].top_api_keys).toHaveLength(6);
		expect(result["gpt-4"].top_api_keys[0].spend).toBe(20.0);
		expect(result["gpt-4"].top_api_keys[5].spend).toBe(15.0);
	});
...
describe("formatKeyLabel", () => {
	...
	it("should use key_name when key_alias is null", () => {
		const modelData = createMockKeyMetricWithMetadata({
			key_alias: null,
			key_name: "named-key",
			team_id: null,
		...
		expect(formatKeyLabel(modelData, "secret-hash-value", MOCK_TEAMS)).toBe("named-key");
	});

	it("should use a truncated hash fallback when alias and name are null", () => {
		const modelData = createMockKeyMetricWithMetadata({
			key_alias: null,
			key_name: null,
			team_id: "team1",
		...
		const result = formatKeyLabel(modelData, "actual-key-secret-value", MOCK_TEAMS);
		expect(result).toBe("actual-key... (team: Test Team 1)");

...
export const columns = (
	...
	{
		header: () => <span className="text-sm font-semibold">Fallback</span>,
		accessorKey: "model_info.fallbacks",
		enableSorting: false,
		size: 180,
		minSize: 100,
		cell: ({ row }) => {
			const model = row.original;
			const fallbacks = Array.isArray(model.model_info?.fallbacks) ? model.model_info.fallbacks : [];
			const canEditFallbacks = userRole === "Admin" && !!onEditFallbacksClick;

			return (
				<div className="flex items-center gap-1 min-w-0 w-full">
					{fallbacks.length > 0 ? (
						<Tooltip title={fallbacks.join(" → ")}>
							<span className="text-xs truncate">{fallbacks.join(", ")}</span>
						</Tooltip>
					) : (
						<span className="text-xs text-gray-400">-</span>
					)}
					{canEditFallbacks && (
						<Tooltip title="Edit fallbacks">
							<button
								type="button"
								aria-label="Edit fallbacks"
								className="text-gray-400 hover:text-gray-600 flex-shrink-0 cursor-pointer"
								onClick={(e) => {
									e.stopPropagation();
									onEditFallbacksClick(model);
								}}
							>
	...
	{
		header: () => <span className="text-sm font-semibold">Health Status</span>,
		id: "health_status",
		enableSorting: false,
		size: 120,
		minSize: 80,
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			const cacheReadInputCost = model.cache_read_input_cost;

			// If both costs are missing or undefined, show "-"
			if (inputCost == null && outputCost == null && cacheReadInputCost == null) {
				return (

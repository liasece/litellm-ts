...
describe("columns", () => {
	...
	describe("fallback column", () => {
		it("should render fallback chain from model_info.fallbacks", () => {
			const cols = columns(
			...
			const model = createMockModel({
				model_info: { ...createMockModel().model_info, fallbacks: ["gpt-4o-mini", "claude-3-7-sonnet"] },
			});
			render(<TestTable data={[model]} columns={cols} />);

			expect(screen.getByText("gpt-4o-mini, claude-3-7-sonnet")).toBeInTheDocument();
		});

		it("should call onEditFallbacksClick when admin clicks edit icon", async () => {
			const onEditFallbacksClick = vi.fn();
			const cols = columns(
				defaultProps.userRole,
				defaultProps.userID,
				defaultProps.premiumUser,
				...
				defaultProps.setExpandedRows,
				undefined,
				onEditFallbacksClick,
			);

			const model = createMockModel();
			render(<TestTable data={[model]} columns={cols} />);

			await userEvent.click(screen.getByRole("button", { name: "Edit fallbacks" }));
			expect(onEditFallbacksClick).toHaveBeenCalledWith(model);
		});

		it("should not render edit icon for non-admin users", () => {
			const cols = columns(
				"Internal User",
				defaultProps.userID,
				defaultProps.premiumUser,
			...
			expect(screen.queryByRole("button", { name: "Edit fallbacks" })).not.toBeInTheDocument();
		});
	});
	...
	it("should display cache read cost when input and output costs are absent", () => {
		...
		render(
			<TestTable
				data={[createMockModel({ input_cost: null, output_cost: null, cache_read_input_cost: "0.25" })]}
				columns={cols}
			/>,
		);

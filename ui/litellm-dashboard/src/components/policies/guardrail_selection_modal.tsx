...
const GuardrailSelectionModal: React.FC<GuardrailSelectionModalProps> = ({
	...
	const guardrailsInfo: GuardrailInfo[] = React.useMemo(
		() =>
			(template?.guardrailDefinitions || []).map((def: any) => ({
		...
		[existingGuardrails, template?.guardrailDefinitions],
	);
	...
	}, [visible, template, guardrailsInfo]);

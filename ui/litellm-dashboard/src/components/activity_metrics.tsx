...
import { valueFormatter, valueFormatterTokens } from "./UsagePage/utils/value_formatters";
...
const VISIBLE_KEY_ROWS = 5;
const KEY_CHART_ROW_HEIGHT = 40;
const KEY_CHART_VERTICAL_PADDING = 34;
const KEY_CHART_MAX_HEIGHT = VISIBLE_KEY_ROWS * KEY_CHART_ROW_HEIGHT + KEY_CHART_VERTICAL_PADDING;

const getTruncatedKeyHash = (keyHash: string): string => `${keyHash.slice(0, 10)}...`;
const getShortKeyHash = (keyHash: string): string => keyHash.slice(0, 8);

const getTopApiKeyLabels = (topApiKeys: TopApiKeyData[]): string[] => {
	const labels = topApiKeys.map((keyData) => {
		const baseLabel = keyData.key_alias || keyData.key_name || getTruncatedKeyHash(keyData.api_key);
		return keyData.team_id ? `${baseLabel} (team_id: ${keyData.team_id})` : baseLabel;
	});
	...
	return labels.map((label, index) =>
		labelCounts[label] > 1 ? `${label} (${getShortKeyHash(topApiKeys[index].api_key)})` : label,
	);
};
...
const ModelSection = ({
	...
	const topApiKeysBySpend = [...topApiKeyChartData].sort((a, b) => b.spend - a.spend);
	const topApiKeysByTokens = [...topApiKeyChartData].sort((a, b) => b.tokens - a.tokens);
			...
			{metrics.top_api_keys && metrics.top_api_keys.length > 0 && (
				<div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
					<Card className="min-w-0">
						...
						<div className="mt-3 overflow-y-auto" style={{ maxHeight: KEY_CHART_MAX_HEIGHT }}>
							<BarChart
								style={{ height: topApiKeysBySpend.length * KEY_CHART_ROW_HEIGHT + KEY_CHART_VERTICAL_PADDING }}
								data={topApiKeysBySpend}
								index="key"
								categories={["spend"]}
								colors={["cyan"]}
					...
					<Card className="min-w-0">
						<Title>Top Virtual Keys by Tokens</Title>
						<div className="mt-3 overflow-y-auto" style={{ maxHeight: KEY_CHART_MAX_HEIGHT }}>
							<BarChart
								style={{ height: topApiKeysByTokens.length * KEY_CHART_ROW_HEIGHT + KEY_CHART_VERTICAL_PADDING }}
								data={topApiKeysByTokens}
								index="key"
								categories={["tokens"]}
								colors={["indigo"]}
			...
			<Grid numItems={2} className="gap-4 mt-4">
				...
				{!hidePromptCachingMetrics && (
					<Card>
						...
						<div className="mb-2">
							<Text>
								Cache Read: {formatNumberWithCommas(metrics.total_cache_read_input_tokens ?? 0, 0, false)} tokens
							</Text>
							<Text>
								Cache Creation: {formatNumberWithCommas(metrics.total_cache_creation_input_tokens ?? 0, 0, false)}{" "}
								tokens
							</Text>
						</div>
						<AreaChart
		...
		<div className="space-y-8">
			...
			<div className="border rounded-lg p-4">
				...
				<Grid numItems={4} className="gap-4 mb-4">
					...
					<Card>
						<Text>Total Tokens</Text>
						<Title>{formatNumberWithCommas(totalMetrics.total_tokens, 0, false)}</Title>
					</Card>
					<Card>
						<Text>Total Spend</Text>
...
export const formatKeyLabel = (modelData: KeyMetricWithMetadata, model: string, teams: Team[]): string => {
	const keyLabel = modelData.metadata.key_alias || modelData.metadata.key_name || getTruncatedKeyHash(model);
	const teamId = modelData.metadata.team_id;
	if (teamId) {
		const teamAlias = resolveTeamAliasFromTeamID(teamId, teams);
		return teamAlias ? `${keyLabel} (team: ${teamAlias})` : `${keyLabel} (team_id: ${teamId})`;
	}
	return keyLabel;
};
...
export const processActivityData = (
	...
	if (key !== "api_keys") {
		Object.entries(modelMetrics).forEach(([model, _]) => {
					...
					Object.entries(modelData.api_key_breakdown || {}).forEach(([apiKey, keyData]) => {
						if (!apiKeyBreakdown[apiKey]) {
							apiKeyBreakdown[apiKey] = {
								...
								key_name: keyData.metadata.key_name,
			...
			modelMetrics[model].top_api_keys = Object.values(apiKeyBreakdown).sort((a, b) => b.spend - a.spend);
		});
	}

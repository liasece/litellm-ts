...
const ModelConnectionTest: React.FC<ModelConnectionTestProps> = ({
	...
	const testInputRef = React.useRef({
		accessToken,
		formValues,
		onTestComplete,
	});
	testInputRef.current = { accessToken, formValues, onTestComplete };

	const testModelConnection = React.useCallback(async () => {
		const {
			accessToken: currentAccessToken,
			formValues: currentFormValues,
			onTestComplete: currentOnTestComplete,
		...
		try {
			console.log("Testing connection with form values:", currentFormValues);
			const result = await prepareModelAddRequest(currentFormValues, currentAccessToken, null);
	...
	}, [testModelConnection]); // Empty dependency array means this runs once on mount

...
import { CredentialItem, CredentialValues, ProviderCredentialFieldMetadata } from "../networking";
...
interface ProviderSpecificFieldsProps {
	...
	editMode?: boolean;
	configuredFieldNames?: ReadonlySet<string>;
}
...
export const createCredentialFromModel = (provider: string, modelData: any): CredentialItem => {
	...
	providerFields.forEach((field) => {
		...
		if (value !== undefined) {
			credentialValues[field.key] = value.toString();
		}
	});
...
const ProviderSpecificFields: React.FC<ProviderSpecificFieldsProps> = ({
	...
	uploadProps,
	editMode = false,
	configuredFieldNames = new Set<string>(),
}) => {
	...
	const fieldPlaceholder = (field: ProviderCredentialField): string | undefined =>
		editMode && configuredFieldNames.has(field.key)
			...
			{allFields.map((field) => (
				<React.Fragment key={field.key}>
					<Form.Item
						...
						name={field.key}
						rules={
							field.required && !(editMode && configuredFieldNames.has(field.key))
					...
					>
						{field.type === "select" ? (
							<Select placeholder={fieldPlaceholder(field)} defaultValue={field.defaultValue}>

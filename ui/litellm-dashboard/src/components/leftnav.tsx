...
const Sidebar: React.FC<SidebarProps> = ({
	...
	const buildMenuItems = (): MenuProps["items"] => {
		...
		menuGroups.forEach((group) => {
			...
			const childMenuItems = filteredItems.map((item) => ({
				key: item.key,
				icon: item.icon,
			...
			// ACCESS CONTROL 组渲染为可折叠 SubMenu（defaultOpenKeys 不含 → 默认收起）；
			// 其余组保持平铺标题（常驻展开，无折叠交互，满足 SETTINGS 默认展开需求）
			if (group.groupLabel === "ACCESS CONTROL") {
				items.push({
					key: "group-access-control",

...
const UserAgentActivity: React.FC<UserAgentActivityProps> = ({ accessToken, userRole, dateValue, onDateChange }) => {
	...
	}, [accessToken, selectedTags, today, userAgentFilter]);

	const fetchWauData = React.useCallback(async () => {
		if (!accessToken) return;
	...
	const fetchMauData = React.useCallback(async () => {
		if (!accessToken) return;
	...
	}, [accessToken, dateValue.from, dateValue.to, selectedTags]);
	...
	useEffect(() => {
		...
		return () => clearTimeout(timeoutId);
	}, [accessToken, dateValue, fetchSummaryData, selectedTags]);

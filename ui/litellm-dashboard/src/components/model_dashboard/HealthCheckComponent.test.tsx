...
describe("HealthCheckComponent", () => {
	...
	it("runs all deployments with one authenticated unparameterized health request", async () => {
		const modelData = {
			data: [
				{ model_name: "first", model_info: { id: "deployment-1" }, litellm_model_name: "first" },
				{ model_name: "second", model_info: { id: "deployment-2" }, litellm_model_name: "second" },
			],
		};
		render(
			<HealthCheckComponent
				accessToken="token"
				modelData={modelData}
				all_models_on_proxy={["deployment-1", "deployment-2"]}
				getDisplayModelName={getDisplayModelName}
			/>,
		);

		await act(async () => screen.getByRole("button", { name: "Run All Checks" }).click());

		await waitFor(() => expect(mockAllDeploymentHealthCheckCall).toHaveBeenCalledTimes(1));
	...
	it("treats a zero-result response as an unhealthy error", async () => {
		mockIndividualModelHealthCheckCall.mockResolvedValue({
			healthy_count: 0,
			unhealthy_count: 0,
			healthy_endpoints: [],
		...
		render(
			<HealthCheckComponent
				accessToken="token"
				modelData={{
					data: [{ model_name: "gpt-4", model_info: { id: "deployment-1" }, litellm_model_name: "gpt-4" }],
				}}
				all_models_on_proxy={["deployment-1"]}
				getDisplayModelName={getDisplayModelName}
			/>,
		...
		await act(async () => screen.getAllByTestId("run-health-check-btn")[0]!.click());

		await waitFor(() => expect(screen.getByText("unhealthy")).toBeInTheDocument());
	});

	it("warns instead of silently completing when no deployment exists", async () => {
		const warningSpy = vi
			.spyOn(Modal, "warning")
		...
		expect(warningSpy).toHaveBeenCalledWith(expect.objectContaining({ title: "No deployments available" }));
		expect(mockIndividualModelHealthCheckCall).not.toHaveBeenCalled();
	});
});
